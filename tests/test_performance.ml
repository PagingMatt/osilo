...
let number_paths = 20
let paths = List.init number_paths ~f:(fun _ -> Printf.sprintf "a/%s" (Nocrypto.Rng.generate 32 |> Coding.encode_cstruct))
let s = "R"
let t =  R
...
let tree' = 
  List.fold ~init:Auth.CS.empty bc_capability
    ~f:(fun s' -> fun (t',c') -> Auth.CS.record_if_most_general s' (t' |> token_of_string) c')

let () = Command.run (Bench.make_command [
  Bench.Test.create_indexed
    ~name:"Building CS tree"
    ~args:(List.range ~start:`inclusive ~stop:`inclusive 1 number_paths)
