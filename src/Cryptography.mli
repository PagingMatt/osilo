...
(** Generic exception to throw when key exchange fails anywhere. TODO add information to this to propogate up stack *)
...
module KS : sig
  ...
  (** Given a keying service, peer, DH group and public key, will carry out DH exchange to return a keying service, private key pair. TODO just return keying service *)
...
module CS : sig
  ...
  (** Given a keying service, peer and Cstruct plaintext return a promise for a tuple containing a keying service (with this peer contained), the Cstruct ciphertext and the Cstruct initial vector. TODO type this in a record? *)

  exception Decryption_failed
  (** In the case that decryption fails, this exception is thrown. TODO encode more information*)
  
  val decrypt : ks:KS.t -> peer:Peer.t -> ciphertext:Cstruct.t -> iv:Cstruct.t -> Cstruct.t
  (** Given a keying service, a peer, a cipher text and an initial vector return the decrypted ciphertext *)
end
